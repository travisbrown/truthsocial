//! Command-line tools for validating and processing Truth Social Wayback Machine archives.
...
/// # Returns
///
/// [`CommandOutcome::ReportedProblems`] if a validation command found an unparseable file or a
/// snapshot that does not match its schema, and [`CommandOutcome::Success`] otherwise
///
/// # Errors
///
/// Returns an error if an input file or database cannot be read, an output file cannot be written,
/// or a CDX entry or snapshot is malformed.
#[expect(
    clippy::too_many_lines,
    reason = "the dispatch is one arm per subcommand"
)]
fn run() -> Result<CommandOutcome, Error> {
    ...
    let outcome = match opts.command {
        Command::ValidateFiles { path, verify } => validate_statuses(&path, verify)?,
        Command::Missing {
            cdx,
            compact,
            limit,
...
/// Validate files in a directory as statuses, status arrays, or account objects.
///
/// Logs parse failures and a summary; undecodable files are counted without individual warnings.
/// With `verify`, also check each file's raw bytes against its Base32 SHA-1 filename.
///
/// # Returns
///
/// [`CommandOutcome::ReportedProblems`] if any file failed, and [`CommandOutcome::Success`]
/// otherwise
fn validate_statuses(path: impl AsRef<Path>, verify: bool) -> Result<CommandOutcome, Error> {
        ...
        .map(|result| {
            result
                ...
                .and_then(|entry| {
                    ...
                    let metadata = path
                        .metadata()
                        .map_err(|e| Error::DirEntry(path.clone(), e))?;
    ...
    Ok(CommandOutcome::from_reported_problems(error_count > 0))
}
...
/// # Returns
///
/// [`CommandOutcome::ReportedProblems`] if either check found a problem, and
...
fn validate_compact(
    ...
    Ok(CommandOutcome::from_reported_problems(!is_successful))
}
...
/// Validates each snapshot's content against the Truth Social [`ResponseBody`] schema.
fn validate_content(path: impl AsRef<Path>) -> Result<ContentValidationSummary, Error> {
    ...
    for result in reader {
        ...
        match result {
            Ok(snapshot) => {
                // Parse content as a `ResponseBody` (a status, an array of statuses, or an
                // account).
                match serde_json::from_str::<ResponseBody<'_>>(snapshot.content.as_str()) {
                    Ok(_) => valid += 1,
                    Err(error) => {
...
/// Validate plain or gzip-compressed JSON as a status, status array, or account.
///
/// A gzip capture is accepted only when the archivindex codec can reproduce it byte-for-byte:
/// `flate2` alone decodes archives whose parameters the codec cannot infer, which would let an
/// unverifiable capture pass. When `verify` is set, the raw bytes must also match the digest
/// filename.
fn validate_single_file(path: impl AsRef<Path>, verify: bool) -> Result<(), FileError> {
    ...
    let _content: ResponseBody<'_> = serde_json::from_str(&content).map_err(FileError::Parse)?;
    Ok(())
}
...
/// memory. A snapshot whose content is an array of statuses contributes every status ID it
/// contains.
...
fn compact(
    ...
    let summary = archivindex_wbm_json_processing::process::compact::compact(
        ...
        CompactConfig {
            ...
            skip_unresolved,
            // Read only the immediate CDX directory.
            cdx_recursive: false,
        },
...
enum Error {
    ...
    Snapshot(#[from] archivindex_wbm_json_processing::io::read::Error),
...
#[derive(Debug, Parser)]
enum Command {
    /// Validate status, status-list, and account JSON files in a directory
    ValidateFiles {
        /// Directory of API response files (plain JSON or gzip-compressed JSON)
        path: PathBuf,
        /// Also verify each file's raw bytes against its digest-named filename (the Base32 SHA-1 of
        /// those bytes). Files whose name is not a valid digest are reported as failures.
        #[arg(long)]
        verify: bool,
    },
    ...
    Merge {
        /// Path to the first zstd-compressed file, sorted by digest bytes
        first: PathBuf,
        /// Path to the second zstd-compressed file, sorted by digest bytes
        second: PathBuf,
    },
    /// Compact raw captures with CDX metadata into a zstd-compressed NDJSON snapshot file
    Compact {
        /// Directory of CDX JSON files (not searched recursively)
        #[arg(long)]
        cdx: PathBuf,
    ...
    /// Print `(url, timestamp, digest)` rows from a CDX CSV whose digest is not in a file of
    /// already-captured digests, in reverse input order
    Next {
        ...
        #[arg(long)]
        cdx: PathBuf,
        /// Maximum number of rows to print (highest status IDs first for sorted `extract` output)
        #[arg(long, default_value = "1000000")]
        count: usize,
    },
    /// Pack raw captures into a zstd-compressed NDJSON snapshot file without CDX metadata
    Pack {
    ...
    /// Add capture timestamps and URLs from a CDX metadata database to a compact snapshot file
    Enhance {
        /// Path to a zstd-compressed compact WTJ snapshot file
        #[arg(long)]
        input: PathBuf,
