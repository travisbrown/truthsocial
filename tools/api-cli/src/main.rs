...
//!     Cloudflare cookies, and any captured bearer token) to the config file.
...
use truthsocial_api::client::Client;
use truthsocial_api::config::Config;
use truthsocial_api::types::SearchKind;
...
/// # Returns
///
/// [`CommandOutcome::Success`] after setup or the requested API operation succeeds.
///
/// # Errors
///
/// Returns an error if the config file cannot be read, the browser session cannot be driven, or the
/// API request fails.
async fn run() -> Result<CommandOutcome, Error> {
    let opts = Opts::parse();
...
#[expect(
    ...
    reason = "the browser session is driven as one linear sequence"
)]
