...
/// Conventional environment variable name for a Cloudflare `cf_clearance` cookie.
/// The client does not read it automatically; pass its value to [`ClientBuilder::cf_clearance`].
...
const DEFAULT_EMULATION: Profile = Profile::Firefox151;

/// The default `User-Agent`, matching a current Linux Firefox so it agrees with both
/// [`DEFAULT_EMULATION`] and a `cf_clearance` cookie obtained from that browser.
...
///
/// Requests are sent through a browser-impersonating HTTP client (see `DEFAULT_EMULATION`) so they
/// pass Cloudflare's bot checks, though a flagged network can still draw a challenge.
///
...
impl Client {
    ...
    /// `GET /api/v2/search?q=`: search accounts, statuses, hashtags, and groups.
    ///
    /// Pass a [`SearchKind`] to restrict the search, or `None` to query all kinds. `resolve` asks
    /// the server to resolve a non-local handle or URL. The parameter is sent as supplied.
    ///
    /// # Errors
    ///
    /// Any request error.
    ...
    ///
    /// Returns the response text without deserializing it as [`SearchResults`], preserving fields
    /// the model does not include. Useful for saving responses or inspecting schema changes.
    ///
    /// # Errors
    ///
    ...
    /// `GET /api/v1/timelines/public`: the public timeline.
    ///
    /// # Errors
    ///
    /// Any request error.
    ...
    pub async fn get_text(&self, path: &str, query: &[(&str, String)]) -> Result<String> {
        ...
        let response = self
            .prepare(self.http.get(url.as_str()).query(query))
            .send()
            .await?;
        let status = response.status();
        let body = response.text().await?;
...
impl ClientBuilder {
    ...
    /// bearer token (if any). Produced by `truthsocial-api setup`. This does not change the
    /// emulation profile; absent optional values leave existing builder settings in place.
    #[must_use]
    ...
    /// Override the browser to emulate (default `DEFAULT_EMULATION`). Pick a different [`Profile`]
    /// if Cloudflare starts flagging the default. This does not change the `User-Agent`, which the
    /// builder always sets (to [`user_agent`](Self::user_agent) or else `DEFAULT_USER_AGENT`), so
    /// override that too to keep the fingerprint and the agent coherent.
    #[must_use]
    pub const fn emulation(mut self, emulation: Profile) -> Self {
        self.emulation = emulation;
        self
    }

    /// Override the `User-Agent` header (default `DEFAULT_USER_AGENT`). It must match the browser a
    /// `cf_clearance` cookie was obtained from, since Cloudflare binds clearance to the agent.
    #[must_use]
