...
//! `<query>.json`: `groups/` holds group search responses and `topics/` holds hashtag search
//! responses. Fixtures are embedded at compile time and asserted to deserialize as
...
//!
//! This mirrors `tests/statuses.rs`; here each payload is a composite `SearchResults` object rather
...
/// Hashtag search fixtures, keyed by `<query>.json` filename.
...
/// Assert that the fixtures embedded in `set` exactly match the files in
/// `tests/data/search/<facet>`.
...
fn topic_search_fixtures_populate_hashtag_history() {
    ...
    // Hashtag search results populate the `hashtags` list.
    assert!(
        results.iter().all(|r| !r.hashtags.is_empty()),
        "a topic fixture parsed with no hashtags"
    );

    // Check that hashtag history and recent usage fields survive deserialization.
    assert!(
