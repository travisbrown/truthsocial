...
//!
//! Each `tests/*.rs` file is its own compilation unit, so code shared between them lives here and
//! is pulled in with `mod common;`. Living under `tests/common/` (rather than as a top-level
