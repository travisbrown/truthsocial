...
//! `<account-id>.json`. Fixtures are embedded at compile time via [`FIXTURES`] and asserted to
//! deserialize as [`Account`], catching any model regression that stops covering them. A second
//! test keeps [`FIXTURES`] and the on-disk directory in sync.
//!
...
/// registered here; [`user_fixtures_directory_matches_enumeration`] fails if the directory and this
/// list diverge.
