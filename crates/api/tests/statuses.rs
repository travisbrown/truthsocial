...
//!
//! Every file in `tests/data/statuses/` is a real Truth Social status API response, named
//! `<status-id>.json`. Fixtures are embedded at compile time via [`FIXTURES`] and asserted to
//! deserialize as [`Status`], catching any model regression that stops covering them. A second test
//! keeps [`FIXTURES`] and the on-disk directory in sync.
//!
//! This mirrors `crates/core/tests/wbm_snapshots.rs`; here the payloads are always plain UTF-8 JSON
//! (never gzip), and each file is a single status object rather than a `StatusContent` that may
//! also be an array.
...
/// Embed a JSON fixture alongside its filename.
macro_rules! fixture {
    ($name:literal) => {
...
/// Every status fixture, keyed by its `<status-id>.json` filename. A new fixture must be registered
/// here; [`status_fixtures_directory_matches_enumeration`] fails if the directory and this list
/// diverge.
...
fn status_fixtures_parse_as_status() {
    for &(name, json) in FIXTURES {
        // Deserialize the embedded JSON fixture.
