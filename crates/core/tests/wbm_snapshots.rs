...
//!
//! Every file in `tests/data/wbm/snapshots/` is a real Truth Social API payload, named by the
//! uppercase Base32 SHA-1 digest of its original bytes. Those bytes are stored exactly as the
//! origin served them, so some are gzip-compressed (magic `1f 8b`). Fixtures are embedded at
//! compile time via [`FIXTURES`] and asserted to deserialize as [`StatusContent`], catching any
//! model regression that stops covering them. A second test keeps [`FIXTURES`] and the on-disk
//! directory in exact sync.
...
/// Embed a snapshot fixture alongside its digest filename.
macro_rules! fixture {
    ($name:literal) => {
...
/// Every snapshot fixture, keyed by its Base32 SHA-1 digest filename. A new fixture must be
/// registered here; [`wbm_snapshots_directory_matches_enumeration`] fails if the directory and this
/// list diverge.
...
/// Decode a raw snapshot payload to its UTF-8 JSON text, transparently gunzipping when the gzip
...
fn wbm_snapshots_parse_as_status_content() {
    for &(name, bytes) in FIXTURES {
        ...
        // Deserialize the decoded snapshot content.
